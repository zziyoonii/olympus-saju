...
(function () {
  ...
  const TEN = {
    ...
    '정재': [21, '상대가 그대의 재(財)의 자리에 든다. 실물과 살림이 맞물리는, 현실이 잘 굴러가는 연이다.'],
  ...
  function stemRel(a, b) {
    if (Math.abs(a - b) === 5) return { label: '천간합 天干合', score: 25, note: '두 일간이 서로를 묶는 합이다. 처음부터 말이 통하고, 떨어져도 다시 이어진다.' };
    if (Math.abs(a - b) === 6) return { label: '천간충 天干沖', score: 9, note: '두 일간이 정면으로 부딪친다. 끌리는 힘도 세지만 밀어내는 힘이 그만큼 세다.' };
  ...
  function branchRel(a, b) {
    ...
    if (s === 1 || s === 13) return { label: '일지 육합 六合', score: 20, note: '태어난 날의 지지가 서로를 감싼다. 살을 붙이고 사는 연에 유리하다.' };
    if (d === 4 || d === 8) return { label: '일지 삼합 三合', score: 17, note: '같은 국(局)을 이루는 지지다. 목표가 같을 때 놀랄 만큼 잘 굴러간다.' };
    if (d === 6) return { label: '일지 충 沖', score: 7, note: '일지가 정면으로 충한다. 가까이 살면 자주 흔들리니, 각자의 방을 두어라.' };
    if (d === 0) return { label: '같은 일지', score: 14, note: '같은 자리에 앉은 두 사람. 생활의 결이 비슷해 편하나 자극이 적다.' };
    if (d === 3 || d === 9) return { label: '일지 형 刑의 결', score: 10, note: '서로를 다듬느라 아프게 하는 결이다. 말의 온도를 낮추어야 오래 간다.' };
    return { label: '일지 무관 無關', score: 12, note: '얽히지도 부딪히지도 않는 자리다. 연은 노력으로 만들어야 한다.' };
  }
  ...
  function elementPart(ea, eb) {
    ...
    return { label: '오행 상보 五行', score, note, combined };
  }
  ...
  function celestialPart(na, nb) {
    ...
    return { label: '천체 각 相位', score, note };
  }
  ...
  function guardianPart(ga, gb) {
    ...
    const ia = E ? E.ROSTER.findIndex((r) => r.god === ga.god) : 0;
    const ib = E ? E.ROSTER.findIndex((r) => r.god === gb.god) : 0;
  ...
  function compare(A, B, tone) {
    ...
    const clash = parts[0].label.indexOf('沖') >= 0 || parts[1].label.indexOf('沖') >= 0;
    ...
    return {
      ...
      strongestLabel: EL_HANJA[strongest] + ' ' + ELS[strongest],
      weakestLabel: EL_HANJA[weakest] + ' ' + ELS[weakest]
    };
  }

  window.OracleCompat = { compare };
})();

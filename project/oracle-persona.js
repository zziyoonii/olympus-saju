/* 신들의 신탁 — 인물 판정 엔진
   ...
   window.OraclePersona 로 노출. oracle-engine.js 이후에 로드할 것 */
(function () {
  const EL = ['목', '화', '토', '금', '수'];
  ...
  const PATTERNS = [
    ...
    {
      k: 'even', w: 60, sec: 0,
      when: (s, saju) => s.zeroEls === 0 && s.heavy.length === 0,
  ...
  function secName(i) {
    const S = window.OracleCopy && window.OracleCopy.SECTIONS;
    return S && S[i] ? S[i].label : '';
  }
  ...
  function read(saju, natal, daeun, birthYear, tone) {
  ...
  window.OraclePersona = { read, structure };
})();

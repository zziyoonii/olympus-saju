/* 신들의 신탁 — 세운·대운·트랜짓 엔진 + 제우스의 연말/신년 신탁
   window.OracleYear 로 노출. oracle-engine.js 이후에 로드할 것 */
(function () {
  ...
  /* ---------- 대운 ---------- */
  function daeun(birth, saju, gender) {
  ...
  function transit(natal, refYear) {
    const E = window.OracleEngine;
    if (!E) return null;
    const y = refYear || new Date().getFullYear();
    const cur = E.natal(y, 12, 31, 12, 0);
  ...
  const TEN_YEAR = {
    '재성': {
      g: '재(財)의 해로 오느니라. 손에 잡히는 것이 늘어나는 자리이니, 늘어난 만큼 새는 곳도 함께 열리는도다.',
      r: '재성의 해다. 실물이 늘어나는 자리이나, 늘어난 만큼 빠져나가는 구멍도 같이 열린다.'
    },
    '관성': {
      g: '관(官)의 해로 오느니라. 네 위에 자리와 책임이 얹히리니, 피하면 도리어 무겁게 눌리리라.',
      r: '관성의 해다. 자리와 책임이 얹히는 구간이니, 피하는 쪽이 오히려 더 무겁게 눌린다.'
    },
    '식상': {
      g: '식상(食傷)의 해로 오느니라. 네 입과 재주가 앞서는 자리이니, 말로 얻고 말로 잃으리라.',
      r: '식상의 해다. 표현과 재주가 앞서는 구간이니, 말로 얻은 것을 말로 잃기 쉽다.'
    },
    '인성': {
      g: '인(印)의 해로 오느니라. 배우고 물러나 정비하는 자리이니, 나아가려 애쓰면 헛발을 딛느니라.',
      r: '인성의 해다. 배우고 정비하는 구간이니, 억지로 밀어붙이면 헛발을 딛는다.'
    },
    '비겁': {
      g: '비겁(比劫)의 해로 오느니라. 어깨를 나란히 할 자도, 네 것을 나눠 가질 자도 함께 오는도다.',
      r: '비겁의 해다. 함께 갈 사람과 내 것을 나눠 가질 사람이 같이 들어온다.'
    }
  };
  const TEN_YEAR_2 = {
    '재성': {
      g: '다시 재(財)가 드느니라. 같은 자리가 두 번 열리는 것은 기회가 아니라 시험이니, 첫 해에 새던 곳을 막지 않았다면 둘째 해에는 더 크게 새리라.',
      r: '재성이 다시 든다. 같은 자리가 두 번 열리는 것은 기회보다 시험에 가깝다. 첫 해에 막지 못한 구멍은 둘째 해에 더 커진다.'
    },
    '관성': {
      g: '다시 관(官)이 드느니라. 지난 해에 얹힌 것을 내려놓지 못했다면, 이 해에는 그 무게로 네 자리가 정해지리라.',
      r: '관성이 다시 든다. 지난 해에 얹힌 것을 내려놓지 못했다면, 이 해에는 그 무게가 네 자리를 정한다.'
    },
    '식상': {
      g: '다시 식상(食傷)이 드느니라. 두 해를 잇는 말은 힘을 얻으나, 두 해를 잇는 실언은 흉으로 굳느니라.',
      r: '식상이 다시 든다. 두 해를 잇는 말은 힘을 얻지만, 두 해를 잇는 실언은 평판으로 굳는다.'
    },
    '인성': {
      g: '다시 인(印)이 드느니라. 두 해를 물러나 있으면 남들은 네가 멈춘 줄 알겠으나, 그 사이 쌓인 것으로 셋째 해를 여느니라.',
      r: '인성이 다시 든다. 두 해를 물러나 있으면 남들은 멈춘 줄 알겠지만, 그 사이 쌓인 것이 셋째 해를 연다.'
    },
    '비겁': {
      g: '다시 비겁(比劫)이 드느니라. 두 해에 걸쳐 사람이 몰리니, 이제는 누구를 남길지 네가 정해야 하리라.',
  ...
  function elementText(yearEl, saju, t, v) {
    ...
    const name = EL[yearEl], h = EL_HANJA[yearEl];
    if (name === weak) {
      if (v === 1) {
        return t === 'g'
          ? `${h} ${name}의 기운이 다시 네 빈 자리로 흘러드느니라. 이 기운을 쥔 사람 곁에 서는 것만으로도 네 몸이 펴이리라.`
          : `${h} ${name}의 기운이 다시 빈 자리로 흘러들어온다. 이 기운을 쥔 사람 곁에 서는 것만으로도 달라진다.`;
      }
      return t === 'g'
        ? `세운의 기운은 ${h} ${name}이니, 네 원국에서 가장 비어 있던 자리를 채우는도다. 이 해에 시작한 것은 뿌리를 얻으리라.`
        : `세운의 기운은 ${h} ${name}. 원국에서 가장 비어 있던 자리를 채운다. 이 해에 시작한 것은 뿌리를 얻는 쪽이다.`;
    }
    if (name === strong) {
      if (v === 1) {
        return t === 'g'
          ? `${h} ${name}${iga(name)} 다시 겹치니, 같은 자리가 두 해 연이어 과해지느니라. 둘째 해에는 더하는 것이 아니라 덜어내는 것이 공이 되리라.`
          : `${h} ${name}${iga(name)} 다시 겹친다. 같은 자리가 두 해 연속 과해지니, 둘째 해에는 더하는 일보다 덜어내는 일이 생산이다.`;
      }
      return t === 'g'
        ? `세운의 기운도 ${h} ${name}이라, 이미 과한 자리에 다시 불을 얹느니라. 넘치는 쪽을 덜어내는 것이 이 해의 과업이로다.`
        : `세운의 기운도 ${h} ${name}. 이미 과한 자리에 더 얹는다. 덜어내는 것이 이 해의 과업이다.`;
    }
    if (v === 1) {
      return t === 'g'
        ? `${h} ${name}의 기운은 두 해에 걸쳐 고르게 흐르니 네 원국을 상하지 않느니라. 판을 바꾸려 하지 말고, 곁에 둘 사람을 바꾸라.`
        : `${h} ${name}의 기운은 두 해에 걸쳐 고르게 흐르고 원국을 상하게 하지 않는다. 판을 바꿀 생각보다 곁에 둔 사람을 바꾸는 쪽이 낫다.`;
    }
    return t === 'g'
      ? `세운의 기운은 ${h} ${name}이니, 네 원국을 크게 흔들지는 않느니라. 흐름을 타되 판을 새로 짜지는 말라.`
      : `세운의 기운은 ${h} ${name}. 원국을 크게 흔들지 않는다. 흐름은 타되 판을 새로 짜지는 마라.`;
  }
  ...
  function reading(saju, natal, gender, birth, tone) {
    ...
    const p2026 = [
      (t === 'g'
        ? `나 크로노스가 ${curY}년 ${A.kr}(${A.ganji})의 남은 ${monthsLeft}달을 셈하노라. `
        : `${curY}년 ${A.kr}(${A.ganji})의 남은 ${monthsLeft}달을 셈한다. `) + TEN_YEAR[GROUP[aTen]][t],
    ...
    const p2027 = [
      (t === 'g'
        ? `해가 바뀌면 ${B.year}년 ${B.kr}(${B.ganji})${iga(B.kr)} 들어서느니라. ` + (GROUP[aTen] === GROUP[bTen]
            ? `자리는 여전히 ${GROUP[bTen]}이니, 바뀌는 것은 기운의 이름이 아니라 그 깊이로다. `
            : `${aTen}에서 ${ro(bTen)} 자리가 바뀌니, 같은 방식으로 살면 같은 결과를 얻지 못하리라. `)
        : `해가 바뀌면 ${B.year}년 ${B.kr}(${B.ganji})${iga(B.kr)} 들어선다. ` + (GROUP[aTen] === GROUP[bTen]
    ...
    if (du && !du.pending) {
      p2027.push(t === 'g'
        ? `너는 지금 ${du.index}번째 대운 ${du.kr}(${du.ganji}) 구간에 서 있으니, ${du.fromAge}세부터 ${du.toAge}세까지 ${du.ten}의 기운이 밑바탕에 깔려 있느니라. 세운은 한 해의 날씨요 대운은 십 년의 기후이니, 날씨를 보고 기후를 잊지 말라.`
        : `지금 ${du.index}번째 대운 ${du.kr}(${du.ganji}) 구간이다. ${du.fromAge}세에서 ${du.toAge}세까지 ${du.ten}의 기운이 밑바탕에 깔린다. 세운은 한 해의 날씨, 대운은 십 년의 기후다.`);
  ...
  window.OracleYear = { reading, yearPillar, daeun, transit };
})();
